...
<script type="text/javascript">
	...
	$toip(document).ready(function(){
		...
		$toip("#resumeCareReasonId").change( function() {
			if ($toip("#resumeCareReasonId").val()=="<spring:message code='tracpatienttransfer.reason.resumeCare.error'/>") {
				$toip("#voidEncounterDivId").show(200);
				$toip("#patientDefaultedBackDivId").hide(200);
	        }else {
				$toip("#voidEncounterDivId").hide();
				if ($toip("#resumeCareReasonId").val()=="<spring:message code='tracpatienttransfer.reason.resumeCare.defaulted.return'/>") {
	...
	function backToParent(){
		if ($toip("#prtSave").val()=="true"){
			$toip("#formTransfer").html("<div onclick='fxCancel()' id='savedDiv'> <spring:message code='tracpatienttransfer.general.careresumed'/> </div>");
